...
int n, m, k, s, t, primMax = 0, capacitateTaietura = 0;
vector<int> tata;
...
//    cout << "DA\n";
    ...
    for (int i = 1; i <= n; ++i) {
        flux += flow[s][i];
        if (isPrime(flow[s][i]) && flow[s][i] > primMax) primMax = flow[s][i];
    }
//    cout << flux << endl;
    afis();
    taietura();
//    cout << capacitateTaietura << endl;
}
...
//    }
    ...
    ek();
    afisare();
    if (primMax > 0) cout << primMax << endl;
    else cout << 1 << endl;
    if (capacitateTaietura == primMax) cout << "Da";
    else cout << "Nu";
